...
snd_sel = 1
local snd_names = {"peak", "decay", "fm low", "rev", "fb", "rate"}
local snd_params = {"peak", "decay", "fm_low_amount", "reverb_mix", "delay_feedback", "delay_rate"}
...
function gridredraw()
  ...
  if edit_ch == 1 or grid_h == 16 then
    for x = 1, 16 do
      if one.data[x] > 0 then
        g:led(x, 9 - one.data[x], 5)
      end
    end
  ...
  if edit_ch == 2 or grid_h == 16 then
    ...
    for x = 1, 16 do
      if two.data[x] > 0 then
        g:led(x, 9 - two.data[x] + y_offset, 5)
      end
    end
  ...
  elseif mode == 1 then --step
    if n == 2 and z == 1 then
      if not alt == true then
        ...
        if edit_ch == 1 then
          edit_ch = 2
          if edit_pos > two.length then
            edit_pos = two.length
          end
        else
          edit_ch = 1
          if edit_pos > one.length then
            edit_pos = one.length
          end
        end
      else
